...
### 吃食物块卡接入敌人侧预览，悬停时显示将吃掉几个食物块
- 做了什么：把“吃食物块”类卡牌也接入了现有的悬停预览链路。现在悬停这类手牌时，除了底部时间轴继续显示时间推进预览，敌人头顶意图牌下方还会额外显示“这张卡将吃到 X 个食物块”提示；同时敌人侧食物槽里将被这张牌吃掉的前排食物块也会出现描边高亮，帮助玩家直接看到具体会吃到哪些块。预览数据复用了 `BattleRules.preview_card_play()`，只做读状态计算，不改真实结算逻辑；如果因为胃容量不足等原因当前不能打出，也仍会保留“若能打出，将吃到 X 个食物块”的参考提示。
- 影响文件：`scripts/runtime/battle_rules.gd`、`scripts/ui/battle_scene.gd`、`scripts/ui/battle_enemy_sprite.gd`、`scripts/ui/food_queue_view.gd`
- 如何验证：进入战斗后，分别悬停 `咬一口`、`咬一大口`、`血盆大口` 之类吃牌，确认敌人头顶意图牌下方会同步显示将吃掉几个食物块；确认敌人侧食物槽前排对应数量的食物块会被高亮圈出；切换不同吃牌时，高亮数量和文案会跟着变化；移开鼠标或取消拖拽后，这些预览应立即消失；若当前胃容量不足导致卡牌不可打出，确认时间轴仍显示不可打出态，敌人头顶提示改为“若能打出，将吃到 X 个食物块”。

### 战斗两侧食物槽改为贴合六边形背包素材，顶部与敌人提示去黑底
- 做了什么：把玩家胃袋/敌人掉落的占位食物块从纵向长条列表改成贴着 `食物槽.png` 六边形孔位的绘制方式，按照素材实际的 `1-2-1-2-1` 排布逐格显示，不再用矩形条目硬盖在背包上。食物槽自身已经提供标题和空槽视觉，所以脚本现在只绘制已有食物块：空槽不再额外写“空位”，食物名和消化时间也不再常驻挤在槽位旁边，只在占用格里用小编号表示顺序。同时把顶部战斗时间条、净化任务条和敌人头顶意图牌统一改成更浅的纸片色 UI，去掉了之前偏重的黑底面板感。
- 影响文件：`scripts/ui/food_queue_view.gd`、`scripts/ui/battle_scene.gd`、`scripts/ui/battle_enemy_sprite.gd`、`scenes/battle/battle_scene.tscn`
- 如何验证：进入战斗后，确认左右两侧只有已有食物块会被绘制到对应六边形槽位中心，空槽不再出现“空位”文字，槽位旁也不再常驻显示食物名/消化时间；确认敌人侧食物块按同一套槽位顺序贴合排列；确认顶部时间条、净化任务区和敌人头顶意图牌都已从深色黑底改为更浅、更贴近现有素材的面板风格。

### 战斗时间改到顶部独立显示，时间轴危险预警加重
- 做了什么：移除了中间提示区里重复的“战斗时间”文案，改为在顶部血条与净化任务之间新增独立时间条，专门显示当前 `t` 进度，减少底部时间轴与中部提示区的重复信息。与此同时，把时间轴里“会撞上敌人行动”的预警从浅红半透明块改成更深的危险红底色加亮色描边，出牌后经过的行动点闪烁颜色也一起加重，让威胁提示更醒目。
- 影响文件：`scenes/battle/battle_scene.tscn`、`scripts/ui/battle_scene.gd`
- 如何验证：进入战斗后，确认当前时间显示在顶部血条与净化任务之间，而不是中间提示区；悬停会跨过敌人行动点的手牌时，确认时间轴上的危险预警比之前明显很多；实际出牌经过敌人行动点时，确认对应标记的闪烁颜色更容易被注意到。

### 战斗中部提示条整体移除
- 做了什么：移除了中间那条用于显示“预计打出…”、“打出…”等文本的提示条，并同步清理脚本中对这块 UI 的刷新逻辑。当前战斗信息重心收敛为：顶部时间、敌人头顶意图、底部时间轴与两侧食物队列，不再让中间提示文本反复争抢注意力。
- 影响文件：`scenes/battle/battle_scene.tscn`、`scripts/ui/battle_scene.gd`
- 如何验证：进入战斗后，确认中间不再出现“预计打出…”或“打出…”的横条提示；悬停手牌和实际出牌时，主要信息应只通过顶部时间、敌人头顶意图和底部时间轴体现。

### 战斗界面移除日志面板并把敌人意图改到头顶显示
- 做了什么：删掉了战斗场景中间大块日志面板，释放出被遮挡的玩家/敌人可视区域。中间信息区收缩为“战斗时间 + 即时提示条”，不再长期堆叠文本记录；敌人下一步行动也不再只写在中间区域，而是改为直接显示在敌人头顶的小意图牌上，让玩家把视线更多留在敌人本体和时间轴之间。
- 影响文件：`scenes/battle/battle_scene.tscn`、`scripts/ui/battle_scene.gd`、`scripts/ui/battle_enemy_sprite.gd`
- 如何验证：进入战斗后，确认中间不再出现大块日志面板，玩家和敌人主体显示区域更完整；确认敌人头顶会显示类似“暗咬（4t 后）”的意图提示，并且当时间推进或敌人行动切换时，这个头顶意图也会同步变化；点击设置、牌堆和弃牌堆按钮时，确认即时提示仍会更新到中间提示条而不是依赖旧日志区。
